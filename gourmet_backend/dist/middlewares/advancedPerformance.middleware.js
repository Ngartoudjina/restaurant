...
exports.resetPerformanceStats = exports.getPerformanceStats = exports.advancedPerformanceLogger = void 0;
const metricsMap = new Map();
const SLOW_THRESHOLD = 200; // ms
// Middleware pour tracker les metrics
const advancedPerformanceLogger = (req, res, next) => {
    ...
    res.json = function (data) {
        ...
        if (duration > SLOW_THRESHOLD) {
            console.warn(`🐌 SLOW REQUEST - ${req.method} ${req.path} [${statusCode}] - ${duration}ms ${cached ? '(cached)' : ''}`);
        }
        else {
            console.log(`✅ ${req.method} ${req.path} [${statusCode}] - ${duration}ms ${cached ? '(cached)' : ''}`);
        }
...
exports.advancedPerformanceLogger = advancedPerformanceLogger;
// Endpoint pour afficher les statistiques
const getPerformanceStats = (_, res) => {
    const stats = {};
    metricsMap.forEach((metrics, endpoint) => {
        ...
        stats[endpoint] = {
            ...
            errorRate: Math.round((metrics.filter(m => m.statusCode >= 400).length / totalRequests) * 100 * 10) / 10
        };
    });
    res.json({
        success: true,
...
// Reset stats
const resetPerformanceStats = (_, res) => {
    metricsMap.clear();
    res.json({
        success: true,
...
//# sourceMappingURL=advancedPerformance.middleware.js.map